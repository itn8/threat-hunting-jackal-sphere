...
1. Defender was chosen and narrowed to use an appropriate table. From the briefing, it’s known that credential stuffing may have been used, so investigating failed logins that then had a success is practical. Using DeviceLogonEvents, at least five failures are searched for before a success is logged to fit a basic brute force pattern.  
